...
class Node:
    ...
    def schedule_next_upload(self, sim: Backup):
        ...
        for peer, block_id in self.remote_blocks_held.items():
            if not self.local_blocks[block_id] and peer.online and peer.current_download is None:
        ...
        if block_id is not None:
            for peer in sim.nodes:
                if peer != self and peer.online and peer.free_space >= self.block_size and peer.current_download is None:
...
def schedule_next_download(self, sim: Backup):
    ...
    # If already downloading, no action needed
    if self.current_download is not None:
        return

    # First, try to find and restore a missing block
    for block_id, peer in enumerate(self.backed_up_blocks):
        if peer and not self.local_blocks[block_id] and peer.online and peer.current_upload is None:
            sim.schedule_transfer(peer, self, block_id, True)
            return  # Exit the function after scheduling a download

    # If no missing block found, try to back up a block for a remote node
    for peer in sim.nodes:
        if (peer is not self 
            and peer.online 
            and peer not in self.remote_blocks_held.values() 
            ...
            if block_id is not None:
                sim.schedule_transfer(self, peer, block_id, False)
                return  # Exit the function after scheduling a backup

    # If no action is taken, the function ends naturally
...
def main():
    ...
    for node_class in config.sections():
        ...
        cfg = {name: parse(class_config[name]) for name, parse in parsing_functions}
